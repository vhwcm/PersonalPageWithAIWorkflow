...
def escolher_modelo():
    """
    Tenta listar os modelos disponíveis para esta chave de API e escolhe
    o primeiro que suporta geração de texto.
    """
    ...
    try:
        ...
        for m in modelos:
            if 'generateContent' in m.supported_generation_methods:
                print(f"⚠️ Modelo escolhido (fallback): {m.name}")
                return genai.GenerativeModel(m.name)

    except Exception as e:
        print(f"⚠️ Não foi possível listar modelos automaticamente: {e}")
    
    # 3. Última tentativa: Hardcoded
    print("⚠️ Tentando forçar 'gemini-1.5-flash-latest'...")
